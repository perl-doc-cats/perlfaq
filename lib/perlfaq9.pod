...
the MIME/QP encoding.  Decoding base 64 becomes as simple as:
...
Use the Email::Sender::Simple module, like so:

  use Email::Sender::Simple qw(sendmail);
  sendmail($email_mime_object);

By default, Email::Sender::Simple assumes that you have a local MTA
(Postfix, Exim, Sendmail, or even qmail) configured to route mail to the
outside world. This generally isn't the case. If there's a remote mail
server you use to send mail, consider investigating one of the Transport
classes. At time of writing, the available transports are:
...
=item Email::Sender::Transport::SMTP::TLS
