...
quoted string C<"variable $variable">, when there isn't whitespace or
punctuation around the variable name you can add braces like
C<"foo ${foo}bar">.
...
L<LIST_SEPERATOR|perlvar/$LIST_SEPARATOR> can be changed by assigning a
different string to the special variable B<$">, such as C<local $" = ', ';>.
...
When you want a list to be expanded per B<$">, use C<@{[ ... ]}>.
