...
To call a function on each integer in a range, use the C<..> range operator:

    my @results = map { some_func($_) } ($min .. $max);

In old versions of Perl, the range operator used to create an
intermediate list in memory; with large ranges, this could be
expensive and therefore an explicit C<for> loop was often preferable.
Since Perl5.005 this is no longer true.
...
you mix C<keys>, C<values>, or C<each> on the same hash, you risk to reset
