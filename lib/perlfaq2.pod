...
Many modules might note their reporting requirements, such as a GitHub
or Google Code tracking system. MetaCPAN (L<https://metacpan.org>) will
show the denoted bugtracker in the left sidebar of a module as the
"Issues" link. You should check the module documentation too.

Failing that, every CPAN module has a bug tracker set up in RT,
L<http://rt.cpan.org>. You can submit bugs to RT either through its web
interface or by email. To email a bug report, send it to I<<
